...
def sim_rmrtt(learner_env, adv_model, output_path, stochastic=False):
    with LogFile(output_path, 'run.log'):
        ...
        for j in range(7,25):
            ...
            learner_env.reset()
            events = []
            adv_state, adv_reward, real_action_cont, real_action, done, learner_info = learner_env.step_adv(0)
            #
            # cc = []
            ...
            for t in range(10):
                ...
                cur_real_action = real_action[np.newaxis]
                cur_real_action_cont = real_action_cont[np.newaxis]
                adv_action[adv_action <= 0] = 0
                # ************************

                adv_state, adv_reward, real_action_cont, real_action, done, learner_info = learner_env.step_adv(adv_action)
                ...
                events.append({'learner reward': learner_info['learner_reward'],
                               ...
                               'real model action': cur_real_action,
                               'real_model action cont': cur_real_action_cont,
                               'adv_state': adv_state})
                t += 1
