...
class DQNAgent:
    ...
    def train(self, env, batch_size=50, total_episodes=200000, output_path=None):
        ...
        env.reset()
        state, _, _, _ = env.step_adv(0) # the input is adv's action
        total_r = 0
        total_psudo_r = 0
        for e in range(total_episodes):
            ...
            action = self.act(state, self.epsilon) # input: RNN's state output: adv's action
            next_state, reward, done, info = env.step_adv(action) # input: adv's action; output: RNN's state, action, and trial information. the second output reward is RNN's action; the reason why it's called reward is because if the targe action is chosen by the RNN learner, it's is a reward for adv
